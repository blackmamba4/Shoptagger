...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      "overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-950/50 backdrop-blur transition hover:border-slate-700/80",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 px-6 py-5 text-left text-base font-medium text-slate-100 transition hover:text-white data-[state=open]:text-white",
        className
      )}
      {...props}
    >
